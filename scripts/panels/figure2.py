...
def _load_stg_hhl():
    stg_path = PROJECT_ROOT / 'data/artifacts' / 'HHL_stg.parquet'
    df = pd.read_parquet(stg_path)
    return df.set_index(['group', 'neuron-id', 'hpr']).query('hpr != -1')


rlfs = _load_rlfs_hhl()
_stg_ids = set(_load_stg_hhl().reset_index()['neuron-id'].astype(str).unique())
_stg_base_ids = {uid[:-1] if uid.endswith(('a', 'b')) else uid for uid in _stg_ids}
rlfs = rlfs.reset_index()
rlfs = rlfs[rlfs['neuron-id'].astype(str).isin(_stg_base_ids)]
rlfs = rlfs.set_index(['group', 'neuron-id', 'hpr', 'lvl']).sort_index()
...
rlfs_norm = rlfs.groupby(['neuron-id', 'hpr']).apply(
    _safe_norm
).droplevel([0, 1])
rlfs_avg = rlfs.groupby(['group', 'hpr', 'lvl']).mean()
...
srlfs_fi = get_groupavg_smooth_fi(rlfs_avg)
df_sig_thresh_gain = _load_stg_hhl()

# %% Panel 2: avg rlfs fi thgd
...
for a in axGRP[2:]:
    a.tick_params(bottom=True, labelbottom=True)
#axGRP[1].margins(x=0.5)

# group panels
a = sns.lineplot(data=rlfs_avg.query('group == "SH"'), x='lvl', y='spks', hue='hpr', palette=palette, ax=axGRP[0], hue_order=hprs, legend=False, errorbar=None)
a.set(ylabel='spk/s',xlabel=None, title='Sham-exposed')
axGRP[0].set_yticklabels(['', '5.5', '11'])

a=sns.lineplot(data=rlfs_avg.query('group == "NE"'), x='lvl', y='spks', hue='hpr', palette=palette, ax=axGRP[1], hue_order=hprs, legend=False, errorbar=None)
a.set(ylabel=None, xlabel=None, title='Noise-exposed')

data=srlfs_fi.reset_index()
data.hpr = data.hpr.astype(int)
data.set_index(['group','hpr','lvl'], inplace=True)

a = sns.lineplot(data=data.loc['SH'].reset_index(), x='lvl', y='fi', hue='hpr', palette=palette, ax=axGRP[2], hue_order=hprs, legend=False, errorbar=None)
...
a = sns.lineplot(data=data.loc['NE'].reset_index(), x='lvl', y='fi', hue='hpr', palette=palette, ax=axGRP[3], hue_order=hprs, legend=False, errorbar=None)
...
df_anim = df_sig_thresh_gain.reset_index().copy()
df_anim['animal'] = df_anim['neuron-id'].astype(str).str.extract(r'^([A-Z]{2}\d+[ab]?)u')
df_anim = (
    df_anim.groupby(['group', 'animal', 'hpr'], as_index=False)[['threshold', 'gain']]
    .mean()
)
df_anim['hpr_str'] = df_anim['hpr'].astype(int).astype(str)
_hpr_order_str = [str(h) for h in hprs_plot]
_group_palette = HHL_GROUP_COLORS.copy()

def _compact_metric_panel(ax, metric, ylabel, ylim, yticks):
    sns.boxplot(
        data=df_anim,
        x='hpr_str', y=metric, hue='group',
        order=_hpr_order_str, hue_order=['SH', 'NE'],
...
_compact_metric_panel(axC_thr, 'threshold', 'Threshold', [48, 81], [50, 60, 70, 80])
# _bracket_y = 0.22
# axC_gain.plot([0, 4], [_bracket_y, _bracket_y], 'k-', lw=1.2)
# axC_gain.text(2, _bracket_y + 0.015, '**', ha='center', va='bottom', fontsize=11)
...
_compact_metric_panel(axC_gain, 'gain', 'Gain', [-0.14, 0.2], [-0.1, 0, 0.1, 0.2])
...
_group_handles = [
    matplotlib.lines.Line2D([0], [0], color=_group_palette['SH'], lw=plt.rcParams['lines.linewidth']),
    matplotlib.lines.Line2D([0], [0], color=_group_palette['NE'], lw=plt.rcParams['lines.linewidth']),
]
axC_leg.legend(_group_handles, ['SH', 'NE'], loc='lower center', bbox_to_anchor=(0.45, -0.225), frameon=False)

# hpr panels
for i, hpr in enumerate(hprs[:-1]):
    i=i+2
    # avg rlf
    ax, data = fig.add_subplot(gs[0,i*2]), rlfs_avg.query(f'hpr == {hpr}')
    if hpr == 30:
        letter_annotation(ax,-.1,1.125,'B')
    ax.set_title(f'HPR c. {hpr}')
    opts = dict(x='lvl', y='spks', ax=ax)
    sns.lineplot(data=data.loc['NE'].reset_index(), color=HHL_GROUP_COLORS['NE'], errorbar=None, **opts)
    sns.lineplot(data=data.loc['SH'].reset_index(), color=HHL_GROUP_COLORS['SH'], errorbar=None, **opts)
    ...
    # density threshold
    data = df_sig_thresh_gain.query(f'hpr == {hpr}').reset_index()
    sns.kdeplot(data=data.query('group == "NE"'),ax=ax_marg_x,x='threshold',color=HHL_GROUP_COLORS['NE'],cut=0)
    sns.kdeplot(data=data.query('group == "SH"'),ax=ax_marg_x,x='threshold',color=HHL_GROUP_COLORS['SH'],cut=0)
    ...
    # density gain
    sns.kdeplot(data=data.query('group == "NE"'),ax=ax_marg_y,y='gain',color=HHL_GROUP_COLORS['NE'],cut=True)
    sns.kdeplot(data=data.query('group == "SH"'),ax=ax_marg_y,y='gain',color=HHL_GROUP_COLORS['SH'],cut=True)
    ...
    # joint plot
    data = df_sig_thresh_gain.query(f'hpr == {hpr}').reset_index()
    sns.scatterplot(data=data.query('group == "NE"'),ax=ax_joint,x='threshold',y='gain', color=HHL_GROUP_COLORS['NE'],s=6)
    sns.scatterplot(data=data.query('group == "SH"'),ax=ax_joint,x='threshold',y='gain', color=HHL_GROUP_COLORS['SH'],s=6)
